...
class ODA:
            ...
            if self.parent:
                for sibling in self.parent.children:
                    if sibling.current_sample == 0:
                        ...
                        if self.keepscore>2:
                            print('*** Idle Sibling Pruned (prune_siblings) ***')
    ...
    ###########################################################################
    def datum_dissimilarity(self, datum, label, recursive=1e3, input_len=None):
    
        y = self.myY[-1]
        
        if input_len!=None: 
            datum = [dd[:input_len] for dd in datum]
            y = [yy[:input_len] for yy in y]
    ...
    ###########################################################################
    def represent(self, datum, recursive=1e3, input_len=None):
        ...
        if recursive>0 and len(self.children) > 0: # and len(self.children[j].myY)>1:
            return self.children[j].represent(datum,recursive-1,input_len=input_len)
        else:
            return self.myY[-1][j], self.myYlabels[-1][j], j
        
    # Find best representative for each class separately
    ###########################################################################
    def represent_separate(self, datum, input_len=None):
    
        yall = self.myY[-1]
        yalllabels = self.myYlabels[-1]

        out = []
        for c in self.classes:

            y = [yall[i] for i in range(len(yall)) if yalllabels[i]==c]
    ...
    ###########################################################################
    def datum_classification_error(self, datum, label, recursive=1e3, input_len=None):
    ...
    ###########################################################################
    def datum_regression_error(self, datum, label, recursive=1e3, input_len=None):
    ...
    # Predict 
    ###########################################################################
    def predict(self, datum, recursive=1e3, input_len=None):
    ...
    def voronoi(self,ctr=-1,points=[],recursive=0,child_ctr=[],add_parent=True):
        ...
        for di in points:
            ...
            j = idx[0]
            regions[j].append(di)
            if len(dists)>1 and np.abs(dists[idx[0]]-dists[idx[1]])<1/4*1e-0*self.myT[ctr]:
                edges.append(di)
